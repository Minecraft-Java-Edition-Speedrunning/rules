...
A.1.1) A launcher that does not significantly modify the game or its files must be used.  
	...
	A.1.1.a) [The vanilla Minecraft launcher](https://www.minecraft.net/en-us/download)  
...
A.1.4.e) Exception: mouseSensitivity may be set to within the range of 0.0 to 1.0.    
A.1.5) The game’s code and files must not be altered to produce any unfair advantage or significant deviation in gameplay.   
A.1.5.a) Clarification: Changing capes and skins in the launcher assets cache is allowed. Skins must not be transparent. Elytra shape must not be changed.  
...
A.2.2 is referenced by: [4.1.2.f](#bookmark=id.r3n8q45ga9to)  
...
A.2.7) The video must end after the run is considered completed.  
...
A.3.1) The F3 screen must be shown during or after the run.  
...
Entangled Rule(s):  
A.3.1 is subject to an amendment by: [H.1.1](#bookmark=id.19zv31onk10m)  
...
A.4.2.c) If you need an accommodation that is not covered by the packs listed above, please open a thread.   
...
A.5.9) If lag slows down the entire run, as is often present within the first few seconds of loading a world in older versions, the extra time will contribute towards in-game time.   
A.5.10) Time spent between segments of a run does not contribute to in-game time.   
...
A.5.11.a) Pauses that occur directly after or close to after (at the discretion of verifiers) chunks of a dimension are loaded for the first time and do not contribute towards in-game time.   
...
A.5.13.b) You may use the option only to show 0-2 decimal places, but all 3 decimal places must be submitted for SpeedrunIGT to be used. This can be done by copying and pasting the text within "speedrunigt/logs/igt\_timer.log" (in the saves folder for the PB) into your run speedrun.com submission’s description.   
...
A.6.4.b) If just Optifine is used (not Fabric Loader or Legacy Fabric Loader), mark your run as Optifine.  
	A.6.4.b.i) Clarification: Optifine may only be used if [A.8.1](#bookmark=id.sfd7fxx1kakg) is followed.  
A.6.4.c) If any other mods are used, including Optifabric, the Fabric Loader, or Legacy Fabric Loader, mark your run as Modded.
...
A.8.1) Optifine is allowed in Minecraft versions 1.0-1.14.4 if and only if the following conditions are met.  
	A.8.1.a) Any sub-menu of the pie-chart (shift+F3) is not activated in 1.14.4.  
...
A.8.2.b) Guidance: Version 0.15.7+ should be used as some mods only support this version of Fabric Loader and above.  
...
A.8.5) Rules relating to what Fabric mods are legal, made illegal, and the conditions of their use are documented on the [legal-mods Github repository](https://github.com/Minecraft-Java-Edition-Speedrunning/legal-mods). Some mods have special conditions of use, such as for use in Set Seed categories only and only for approved users on the basis of accessibility, as described in the README.md.  
...
A.8.7.i) Exception: Any version of WorldPreview before 2.3.3 must not be used.  
...
A.12.1.b) Attempting to gain information about a random seed through disallowed programs  
...
D.1.1.s) Boat placing inside of blocks ([Example](https://youtu.be/ulIljEGxujI))  
...
D.1.2.n) Desynced lava blip ([MC-12363](https://bugs.mojang.com/browse/MC-12363))  
...
D.1.3.b) Exception: Runners may Save and Quit within a run intentionally if and only if there is a bug which can be resolved only (at the discretion of verifiers) by saving and quitting (ex. pie chart bug). No additional benefit may be gained.  
...
Entangled Rule(s):  
D.1.3 is nullified by: [4.1.4](#bookmark=id.fg6g86gnr0mm)  
D.1.3 is referenced by: [E.3.1](#bookmark=id.20y1amfehm7), [E.4.1.a](#bookmark=id.6sbqd7lvjgkg), [4.1.2.f](#bookmark=id.r3n8q45ga9to)  
...
H.1.1) The title screen can be shown in lieu of the F3 screen, as mandated by [A.3.1](#bookmark=id.8xgunvecshzx). The bottom left of the title screen must be legible if the F3 screen is not shown.   
...
## **1.1: Objective and Clarifications**

1.1.1) The run is considered completed once the end credits of the game begin after entering the exit end portal.   
Entangled Rule(s):  
...
## **2.2: Any% Internal Random Seed Pre-1.9**
...
## **2.3: Any% Internal Random Seed 1.9+**
...
## **4.1: All Advancements Objective, Exceptions, and Clarifications**
...
4.1.1) The run is considered complete once every advancement has been achieved.   
	4.1.1.a) Timing ends once the notification for the last advancement is shown.  
4.1.2) A runner may segment their run, thereby nullifying [A.2.2](#bookmark=id.kibxavdg498o) and [D.1.3](#bookmark=id.mjw1q0inxd8v), if and only if the following pretences are met:  
...
5.1.1) The run is considered complete under the same pretences as Any% Glitchless (see [1.1.1](#bookmark=id.mgq421lsycms)).
...
6.1.1) The objective, exceptions, and clarifications of All Advancements Co-op are identical to those of All Advancements (see section [4.1](#bookmark=id.1ardfzk8z2na)).  
6.1.1.a) If the run is segmented, rather than not playing on the instance between segments ([4.1.2.c](#bookmark=id.fo0l93wnfy45)), the server folder must not be played between segments.  
6.1.1.b) If the run is segmented, you must submit server logs instead of instance logs ([4.1.2.d](#bookmark=id.c5ps02o6wkyf)).  
...
6.1.3) The list of all advancements is the same as for All Advancements (see [4.6.1](#bookmark=id.yaevtzjjjv5h) and [4.7.1](#bookmark=id.c6d4ai8u3bap)).  
...
7.1.1) The run is considered complete under the same pretences as Any% Glitchless (see [1.1.1](#bookmark=id.mgq421lsycms)).  
...
7.1.3.c) In addition to the world generation settings allowed in [A.1.3](#bookmark=id.6vsi9vckzfzn), you must also enable the Bonus Chest.  
...
8.1.1) The run is considered complete under the same pretences as Any% Glitchless (see [1.1.1](#bookmark=id.mgq421lsycms)).  
...
9.1.1) The run is considered complete under the same pretences as Any% Glitchless (see [1.1.1](#bookmark=id.mgq421lsycms)).  
9.1.2) Any% Peaceful follows identical rulesets regarding peaceful difficulty as Any% Glitchless Peaceful (see [8.1.2](#bookmark=id.tlf9vzvpdol4)\-[8.1.3](#bookmark=id.1d34gwtcuk9w)).  
...
11.1.1) The run is considered complete under the same pretences as Any% Glitchless (see [1.1.1](#bookmark=id.mgq421lsycms)).  
11.1.2) Sections [A.5](#bookmark=id.s9xaud4bhklt) and [E.2](#bookmark=id.cmrzt4p17win) are nullified.  
...
11.1.4) The run must employ some version of time travel: where the game is crashed and reverted to a previous state.   
...
CE1.1.1.b) If the last boss killed is the ender dragon, timing ends as per Any% Glitchless standards (see [1.1.1](#bookmark=id.mgq421lsycms)).  
...
CE2.1.1) The run is considered complete once the How Did We Get Here? advancement has been achieved.   
	CE2.1.1.a) Timing ends once the notification for How Did We Get Here? is shown.  
...
CE3.1.1) The run is considered complete once the Hero of the Village advancement has been achieved.   
	CE3.1.1.a) Timing ends once the notification for Hero of the Village is shown.  
...
CE4.1.1) The run is considered complete once the Cover Me in Debris advancement has been achieved.   
	CE4.1.1.a) Timing ends once the notification for Cover Me in Debris is shown.  
...
CE5.1.1) The run is considered complete once the Arbalistic advancement has been achieved.   
	CE5.1.1.a) Timing ends once the notification for Arbalistic is shown.  
...
CE6.1.1) The run is considered complete once the item in question has been obtained.   
	CE6.1.1.a) Timing ends once the item is obtained (see [A.5.18](#bookmark=id.dav11hwprkub)).  
...
CE8.1.1) The run is considered complete once all swords have been simultaneously obtained.  
	CE8.1.1.a) Timing ends once the last sword is obtained. (see [A.5.18](#bookmark=id.dav11hwprkub))  
...
CE8.2.1) The run is considered complete once a wood sword, stone sword, iron sword, gold sword, and diamond sword are obtained.

## **CE8.2: Set Seed Glitchless/Random Seed Glitchless, Structures/No Structures, 1.16+**
...
## **CE9.7: Set Seed Glitchless/Random Seed Glitchless, 1.20+**
...
CE10.1.1) The run is considered complete once all colours of wool have been simultaneously obtained.  
	CE10.1.1.a) Timing ends once the last colour of wool is obtained. (see [A.5.18](#bookmark=id.dav11hwprkub))  
Entangled Rule(s):  
CE10.1.1 is referenced by: [CE21.1.1](#bookmark=id.rz3jyxxg1dhm)  
CE10.1.2) The list of all wool colours are: white wool, orange wool, magenta wool, light blue wool, yellow wool, lime wool, pink wool, grey wool, light grey wool, cyan wool, purple wool, blue wool, brown wool, green wool, red wool, and black wool.  
Entangled Rule(s):  
CE10.1.2 is referenced by: [CE21.1.1](#bookmark=id.rz3jyxxg1dhm)  
...
CE11.1.1) The run is considered complete once all minerals have been simultaneously obtained.  
	CE11.1.1.a) Timing ends once the last mineral is obtained. (see [A.5.18](#bookmark=id.dav11hwprkub))  
...
Entangled Rule(s):  
CE12.1.1 is referenced by: [CE22.1.1](#bookmark=id.gf82x7y26f3)  
...
CE13.1.1) The run is considered complete once all workstations have been simultaneously obtained.  
	CE13.1.1.a) Timing ends once the last workstation is obtained. (see [A.5.18](#bookmark=id.dav11hwprkub))  
...
CE14.1.1.b) Clarification: All 36 inventory slots must be filled, but the offhand and armour slots do not have to be filled.  
...
CE16.1.1.b) Timing ends once the player completes the final version, as per Any% Glitchless standards (see [1.1.1](#bookmark=id.mgq421lsycms)).  
CE16.1.2) Clarification: Players must complete each version consecutively (1.0, 1.1, 1.2, 1.3…).  
CE16.1.3) Clarification: Any subversion may be chosen for each main version.  
CE16.1.4) The category is timed by RTA, nullifying [A.5](#bookmark=id.s9xaud4bhklt).  
...
CE16.2.1) Select F3 if your run uses F3 (as defined in [A.6.2](#bookmark=id.xeaaix8s3pue)) for any run.  
...
CE16.4.1) Runs must use Easy/Normal/Hard for at least one run.  
CE16.4.2) Mark your run as the lowest difficulty you used for any run at any point.

## **CE16.5: Hardcore**

CE16.5.1) Runs must use the Hardcore difficulty for every run.  
	CE16.5.1.a) Clarification: Runs which are reset should be in hardcore as well.

# **CE17: Mine a Chunk**

## **CE17.1: Mine a Chunk Objective and Clarifications**

Entangled Rule(s):  
CE17.1 is referenced by: [CE17.2.1](#bookmark=id.u020u6319hka), [CE17.3.1](#bookmark=id.803nkh8cbwgq)  
...
CE17.1.2) You must mine every level at which bedrock does not appear.  
...
CE17.2.1) The run is considered complete once a chunk is mined (see [CE17.1](#bookmark=id.xbz9953efw4g)) in the overworld.

## **CE17.3: RSG/SSG/RS/SS, Pre-1.17/1.17/1.18+, Nether, Solo/Duos/3-4/5-9/10+**

CE17.3.1) The run is considered complete once a chunk is mined (see [CE17.1](#bookmark=id.xbz9953efw4g)) in the nether.
...
CE18.1.1) The run is considered complete once 64 lime wool has been obtained.  
	CE18.1.1.a) Timing ends once the last piece(s) of lime wool are obtained. (see [A.5.18](#bookmark=id.dav11hwprkub))  
...
CE19.1.1) The run is considered complete once the game is beaten after all end portals have been lit.  
	CE19.1.1.a) Timing ends as per Any% Glitchless timing (see [1.1.1](#bookmark=id.mgq421lsycms)).  
CE19.1.2) All runs begin upon world load.

## **CE19.2: All Portals Pre-1.9**

CE19.2.1) The run is considered complete once the game is beaten after all 3 end portals have been lit.  
CE19.2.2) You must not enter the end until all 3 end portals have been lit. 

## **CE19.3: All Portals 1.9+**

CE19.3.1) The run is considered complete once the game is beaten after all 128 end portals have been lit.  
...
CE19.4.3) Purist runs must not use F3 as defined by [A.6.2](#bookmark=id.xeaaix8s3pue).
...
CE20.1.2) After the random seed is generated, you have 10 minutes RTA, beginning on world load, to scout the seed. During this time and this time only, cheats and open to LAN may be used, nullifying [A.3.3](#bookmark=id.d8b2c7okkc3h), and [A.3.5](#bookmark=id.5il6rk51l0p0). Recreating the world during this time is also permitted. In addition to the world generation settings allowed in [A.1.3](#bookmark=id.6vsi9vckzfzn), you may also enable cheats.  
CE20.1.2.a) Any resource packs may be used for the scouting section of the run only (nullifying [A.4.4](#bookmark=id.962qtzyj8xj3)).  
...
CE20.1.3.a) Clarification: The world generation settings must follow normal world generation settings ([A.1.3](#bookmark=id.6vsi9vckzfzn)) and any rules which were previously nullified are now reinstated.  
...
CE20.2.2) Clarification: External tools, outside of those outlined by [A.11.7](#bookmark=id.3mfjhnqzy4al), must not be used.  
CE20.2.3) Macros may be used during the scouting time only (nullifying [A.10.4](#bookmark=id.2s10wtg93l2e)). However, they must only execute /teleport (/tp), /locate, /setblock, and /gamemode commands.  
...
CE21.1.1) All Wool (Glitched) follows the same pretences for completion as All Wool (see [CE10.1.1](#bookmark=id.jnfn4tw8yk0)\-[CE10.1.2](#bookmark=id.yxfwcgyhyk3s)).  
...
CE22.1.1) Full Iron Armor and 15 Levels (Glitched) follows the same pretences for completion as Full Iron Armor and 15 Levels (see [CE12.1.1](#bookmark=id.vhxcvknasoa3)).  
...
CE24.1.1) The run is considered complete once exactly 40 advancements, as listed in [4.6.1](#bookmark=id.yaevtzjjjv5h),  have been obtained.  
	CE24.1.1.a) Timing ends once the notification for the last advancement is shown.  
...
CE25.1.2) The four flags you must create must have the following colours:  
	...
	CE25.1.2.e) Showcase: An [image](https://imgur.com/shBryrT) of the colours which need to be included in the flag.  
...
CE26.1.1) The run is considered complete once the player reaches a Y level of 420\.  
...
CE26.1.2.a) Exception to [A.6.2](#bookmark=id.xeaaix8s3pue): If your run only uses F3 to display that you have met the run’s objective, your run will be marked as No F3.  
